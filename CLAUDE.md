...
Scoring always uses the **90-minute (regular-time) score** — see "Extra time / penalties" below for
where that comes from for knockout matches, where `fullTime` is the final aggregate instead.
...
- **Extra time / penalties**: football-data.org's `score.fullTime` field is the *final* match score for a knockout match — for matches with `score.duration !== 'REGULAR'` that can include extra-time goals, not the 90-minute score our rules require. But v4 also exposes `score.regularTime` (the 90-minute score) plus `score.extraTime`/`score.penalties` (goals scored in ET / the shootout result), so these matches ARE auto-scored: `pickRegularTimeScore()` (`lib/football-data/regular-time.ts`, mirrored inline in the `poll-match-results` edge function) picks `regularTime` over `fullTime` for any `duration !== 'REGULAR'` match and feeds it to `scoreMatchAndNotify()` same as a group match. It also fills `matches.winner_team` from `score.winner`, `matches.went_to_extra_time`/`result_duration` from `duration`, and `matches.extra_time_home/away`, `matches.penalties_home/away` for display (migration 0027) — none of the ET/penalty fields ever feed point scoring. If a knockout match is `FINISHED` but `regularTime` hasn't appeared yet (rare upstream lag), the poller skips it and retries on the next run rather than guessing from `fullTime`. `matches.needs_manual_score` / `/admin/matches` override / `/setscore <id> <h-a> [koti|vieras]` remain as a manual fallback (e.g. if football-data.org data is ever wrong) — `winner_team` is still required there for any knockout-stage draw (enforced client-side in `/admin/matches` and server-side in `POST /api/admin/override-result`), since it feeds the playoff bracket and elimination-highlighting features.
